...
- **Long content**: a session name, folder/file path, or model name has no enforced length limit upstream — test every such label at ~200 characters, not just realistic short ones. Single-line labels (session card title, breadcrumb segment) truncate with an ellipsis (`numberOfLines={1}`); multi-line contexts (a body/description) wrap normally rather than truncating. Never let a long label push a card's layout wider or break a fixed-height row.
- **RTL — not decided, not built.** No screen has been checked against a right-to-left locale, and `I18nManager`/`writingDirection` don't appear anywhere in the app. This is an open gap, not a considered "we don't support RTL" — flag it before assuming either answer, same status as the high-contrast gap in `pig-color-system`.
- **Emoji in rendered content is not the same thing as emoji in UI-authored copy.** The no-emoji rule repeated across every `pig-*` skill governs text *this app writes* — button labels, dialog copy, empty states. It does not mean stripping emoji out of an agent's markdown response or a user-visible filename; those are content, rendered as-is through `pig-markdown-rendering`'s normal text path. Don't add emoji-stripping logic to the renderer to satisfy the no-emoji rule — that rule was never about content.
